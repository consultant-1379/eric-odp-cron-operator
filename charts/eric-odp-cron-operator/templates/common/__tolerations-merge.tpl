...
*/}}
{{- define "eric-odp-cron-operator.merge-tolerations" -}}
  {{- if (.root.Values.global).tolerations }}
      ...
      {{- range $globalItem := $globalTolerations -}}
        {{- $globalItemId := include "eric-odp-cron-operator.merge-tolerations.get-identifier" $globalItem -}}
        {{- range $serviceItem := $serviceTolerations -}}
          {{- $serviceItemId := include "eric-odp-cron-operator.merge-tolerations.get-identifier" $serviceItem -}}
      ...
      {{- range $globalItem := $globalTolerations -}}
        ...
        {{- range $matchItem := $matchingItems -}}
          {{- $matchItemId := include "eric-odp-cron-operator.merge-tolerations.get-identifier" $matchItem -}}
...
*/}}
{{ define "eric-odp-cron-operator.merge-tolerations.get-identifier" }}
  {{- $keyValues := list -}}
